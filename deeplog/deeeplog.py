# shoutout to https://github.com/nailo2c/deeplog.
# I copied this code from him. was easier than forking
import os
import sys
import json
import logging
...
def _get_train_data_loader(batch_size, is_distributed, window_size, local, **kwargs):
    ...
    _generate = Generate()
    seq_dataset = _generate.generate(name='../data/parsed/train',
                                     window_size=window_size, local=local)
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        seq_dataset) if is_distributed else None
...
def train(args):
    ...
    if args.num_gpus > 0 and not torch.cuda.is_available():
        logger.warning(
            "No CUDA available, setting num_gpus to 0 (num_gpus = {}).".format(args.num_gpus))
        args.num_gpus = 0
    use_cuda = args.num_gpus > 0
    logger.debug("Number of gpus requested - {}, available - {}.".format(args.num_gpus,
    ...
    train_loader = _get_train_data_loader(
        args.batch_size, is_distributed, args.window_size, args.local, **kwargs)
