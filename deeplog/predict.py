...
if __name__ == '__main__':
    ...
    ##############
    model_dir = './model'
    model_info = deeeplog.model_fn(model_dir)
    logger.debug("Number top candidates: ", model_info["num_candidates"])
    logger.debug("Input_size: ", model_info["input_size"])
    logger.debug("Window_size: ", model_info["window_size"])
    ...
    with open('test_abnormal', 'r') as f:
        for line in f.readlines():
            ...
            response = deeeplog.my_fast_predict_fn(line, model_info)
            test_abnormal_list.append(response)

    logger.info("Finished abnormal List")
